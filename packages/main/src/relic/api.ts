...
class Api {
    public async getUser(steamId: string) {
        try {
            const {data} = await axios.get('/relicLink.php', {
                params: {playerSteamSummary: steamId},
            });
            const stats = await this.getStats(steamId);

            return {
                ...data.steamResults.response.players[0],
                ...{stats: stats},
    ...
    public async getProfile(steamId?: string) {
        const {data} = await axios.get('/relicLink.php', {
            params: {steamUserID: steamId || app.settings.steamId},
        });
