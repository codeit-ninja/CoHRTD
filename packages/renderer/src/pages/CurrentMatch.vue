<template>
    <div class="container mx-auto">
        ...
        <hr class="my-5" />
        <div v-if="gameStore.game" class="teams teams-container">
            <div class="row g-0 align-items-center">
                <div class="col-3">
                    <h2 class="text-end mb-4">Allies</h2>
                    <span v-for="(player, index) in gameStore.game?.teams[0]" :key="index" class="d-flex gap-2 align-items-center justify-content-end">
                        <span class="teams-player-name small">{{ player.playerId === -1 ? 'CPU' : player.alias }} {{ player.difficulty ? ' - ' + player.difficulty : '' }}</span>
                        <RankIcon :rank="player.rank ?? 0" :faction="Factions.Us" />
                    </span>
                </div>
                <div class="col-6 text-center">
                    <img :src="VersusVector" width="100" class="versus-vector" />
                </div>
                <div class="col-3">
                    <h2 class="mb-4">Axis</h2>
                    <span v-for="(player, index) in gameStore.game?.teams[1]" :key="index" class="d-flex gap-2 align-items-center">
                        <RankIcon :rank="player.rank ?? 0" :faction="Factions.Us" />
                        <span class="teams-player-name small">{{ player.playerId === -1 ? 'CPU' : player.alias }} {{ player.difficulty ? ' - ' + player.difficulty : '' }}</span>
                    </span>
                </div>
